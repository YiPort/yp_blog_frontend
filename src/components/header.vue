...
<template>
  <div>
    <div id="nav-bar" :class="navShow ? 'navOn headBack' : 'navOff headBack'">
          ...
          <div class="headBox">
            ...
            >
              ...
              <el-submenu index="/System" v-show="isAdmin">
                ...
                >
                <el-menu-item index="/System/Article"
                  ><i class="el-icon-tickets" />文章管理</el-menu-item
                >
                <el-menu-item index="/System/Comment"
                  ><i class="el-icon-chat-line-round" />评论管理</el-menu-item
                >
                <el-submenu index="/LoginInfo">
                  <template slot="title"
                    ><i class="el-icon-setting"></i>访问管理</template
                  >
                  <el-menu-item index="/System/LoginInfo"
                    ><i class="el-icon-postcard" />用户访问管理</el-menu-item
                  >
                  <el-menu-item index="/System/Ip"
                    ><i class="el-icon-data-analysis" />访问统计</el-menu-item
                  >
                </el-submenu>
                <el-menu-item index="/System/Cache"
                  ><i class="el-icon-coin" />缓存监控</el-menu-item
                >
              </el-submenu>
              <el-menu-item style="margin-right: 15px" index="/Write">
                <i
                  style="color: #fff; margin-right: 0"
                ...
                <div v-show="haslogin" class="haslogin avatarClass">
                  ...
                  >
                    ...
                    <el-dropdown-menu slot="dropdown">
                      ...
                      <a href="#/Help">
                        <el-dropdown-item icon="el-icon-service"
      ...
      <div class="h-information">
        <a :href="userInfo ? '#/UserInfo' : '#/'">
          <img
            ...
            :src="userInfo.avatar ? userInfo.avatar : 'static/img/tou.png'"
            alt=""
          />
        </a>
  ...
  methods: {
    ...
    userlogout() {
      this.$confirm("是否确认退出?", "退出提示", {
        ...
        logout().then((response) => {
          ...
          this.$store.commit("changeReloadUserInfo", true);
          this.haslogin = false;
          this.isAdmin = false;
          this.$message({
            type: "success",
    ...
    searchArticle() {
      this.$prompt("请输入搜索的关键字", "搜索文章", {
        ...
        this.$router.push({
          path: "Search",
          query: { searchKey: this.queryParams.searchKey },
        });
      });
      //   .catch(() => {
      //   this.$message({
    ...
    routeChange() {
      ...
      if (localStorage.getItem("userInfo")) {
        ...
        this.isAdmin = this.userInfo.userRole === "1" ? true : false;
      } else {
        this.haslogin = false;
        this.isAdmin = false;
        this.userInfo = "";
    ...
    querySearchAsync(queryString, cb) {
      ...
      var results = queryString
        ? allTitle.filter(this.createStateFilter(queryString))
        : allTitle;
...
/* 0-1前段动画 */
.headBox .el-input__inner:hover,
.el-autocomplete:hover + .headBox .el-input__inner:focus {
  height: 30px;
  width: 270%;
  border: none;
  background: rgba(40, 42, 44, 0.178);
...
/* 1-2后段动画 */
.headBox .el-input__inner:focus,
.el-autocomplete:hover + .headBox .el-input__inner {
  visibility: visible !important;
  opacity: 2;
  cursor: pointer;
}
...
.headBox .searchArticle {
  ...
  line-height: 38px;
  position: absolute;
  right: -90px; /* Adjusted from 1px to -60px to move it to the right */
  top: 0;
  color: #fff;
}
