 ...
 <div class="detailBox tcommonBox" >
    <el-skeleton :rows="20" animated :loading="loading">
      ...
      <template>
        ...
        <header>
          <a :href="'#/Write'" target="_blank" v-show="isMy">
            <el-button style="left: 90%;position: relative;color: black"
            @click="editArticle" v-show="isMy" round>
              编辑</el-button>
          </a>
            <h1>
...
import { postArticleIndex } from '../api/search.js'
...
export default {
  ...
  watch: {
    ...
    createBy() {
      this.isMy = (this.createBy && this.createBy === this.userId) ? true : false;
      // console.log(isMy)
      this.$store.commit('changeIsMy', this.isMy)
      this.$store.commit('changeCreateBy', this.createBy)
    }
  },
  components: { //定义组件
